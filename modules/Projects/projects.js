...
module.exports = function (app, io) {
    ...
    app.get('/projects2/:Id', (req, res, next)=>{
      isLoggedIn(req, res, next, 'manageProject')
    }, function (req, res) {
        ...
        isLoggedIn(req, res, next, 'manageProject')
          ...
          User.findOne({Id: req.decoded.user}, {Password: 0}, function (err, user) {
              ...
              if (user) {
                  ...
                  User.find({team_Id: user.team_Id}, function (err, userDet) {
  
                      var Team = userDet;
                      data.people = Team.length;
                      data.active = 0;
                      data.completed = 0;
                      var search = {company_Id: user.team_Id};
                      if (user.type == 2) {
                          search.manager_Id = user.Id;
                      }
                      Projects.find(search, function (err, projects) {
                          ...
                          if (parseInt(user.type) == 1) {
                              ...
                              query = {team_Id: {$in: [req.params.Id]}};
                              // console.log(query);
                          }
                          ...
                          Projects.find(query, function (err, projects) {
                            ...
                            User.find({
                                team_Id: projects.length == 0?0:projects[0].company_Id,
                                type: {$ne: 1}
                            }, function (err, client) {
                                var team = {};
                                var updatedProject = [];
                                ...
                                for(var y in client){
                                    team[client[y].Id] = client[y];
                                }
                                for(var i in projects){
                                    updatedProject[i] = projects[i].toJSON();
                                    updatedProject[i].members = [];
                                        for(var user_id in updatedProject[i].team){
                                            if(!team[updatedProject[i].team[user_id]]){
                                                continue;
                                            }
                                            let user = team[updatedProject[i].team[user_id]];
                                            let member = {name: user.first_name + " "+ user.last_name, profile_thumbnail: user['profile_thumbnail']};
                                            member.Id = user.Id;
                                            updatedProject[i].members.push(member); 
                                        }
                                }
                                    res.send({
                                        status: true,
